...
class Book(Base):
    ...
    id = Column(UUID, primary_key=True, default=uuid.uuid4, server_default=text('gen_random_uuid()'))
    author_id = Column(UUID, ForeignKey('author_profiles.id'), nullable=True)
    ...
    likers = relationship("User", secondary="book_likes", back_populates="liked_books")
...
class Review(Base):
    ...
    likers = relationship("User", secondary="review_likes", back_populates="liked_reviews")
